module Kryptonite
  ...
  protected
    ...
    def add_namespace_to_routes
      puts "   kryptonite     adding namespace to routes.rb"
      file_to_update = Rails.root + 'config/routes.rb'
      line_to_add = "namespace :kryptonite do"
    ...
    def add_to_routes
      puts "   kryptonite     adding #{plural_name} resources to routes.rb"
      file_to_update = Rails.root + 'config/routes.rb'
      line_to_add = "resources :#{plural_name}"
      insert_sentinel = 'namespace :kryptonite do'
      gsub_add_once plural_name, file_to_update, "\t\t" + line_to_add, insert_sentinel
    end

    def add_to_navigation
      puts "   kryptonite     adding #{plural_name} to left navigation bar"
      file_to_update = Rails.root + 'app/views/kryptonite/layouts/_left_navigation.html.erb'
      line_to_add = "<li id=\"visitSite\"><%= link_to \"#{plural_name.humanize.capitalize}\", kryptonite_#{@plural_route}_path %></li>"
